...
class PubMedUtils(object):
    ...
    def getJournalIndex(self, filePath):
        ...
        with open(filePath, 'r') as ifh:
            for line in ifh:
                if '----' in line and 'journal_title' in d:
                    logger.debug("line %r: %r" % (line, d))
                    rL.append(copy.copy(d))
                    d = {}
                #
                fields = [f.strip() for f in line[:-1].split(':')]
                #
                if fields[0] == 'JournalTitle':
                    d['journal_title'] = fields[1:]
                if fields[0] == 'IsoAbbr':
                    d['iso_abbrev'] = fields[1:]
                if fields[0] in ['ISSN (Print)']:
                    d['issn_print'] = fields[1:]
                if fields[0] in ['ISSN (Online)']:
                    d['issn_online'] = fields[1:]
        #
