...
class PubMedUtilsTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.__pidList2 = ['27744351', '30413611', '10360352', '29234012',
                           '30254335',
                           '16041081',
                           '8341712',
                           '30746096',
    ...
    def testFetchIds(self):
        ...
        try:
            fobj = PubMedUtils(saveText=True)
            for id in self.__pidList1:
                idList = [id]
                retD = fobj.fetchList(idList)
                self.assertGreaterEqual(len(retD), len(idList))
                #
                if retD and self.__export:
                    fobj.writeUnpXml(os.path.join(self.__workPath, id + '.xml'))
                    self.__mU.doExport(os.path.join(self.__workPath, id + '.json'), retD, format='json', indent=3)
                #
        except Exception as e:
            logger.exception("Failing with %s" % str(e))
            self.fail()
